...
PerspectiveProgram::PerspectiveProgram(const std::string& vertexShaderFile,
...
void PerspectiveProgram::setPositionColor(const sharedlib::VertexArray& va)
{
...
void PerspectiveProgram::setModelMatrix(const sharedlib::Matrix4& matrix)
{
    glUniformMatrix4fv(modelMatrixLocation_, 1, GL_TRUE,
                       reinterpret_cast<const GLfloat *>(matrix.data()));
}

void PerspectiveProgram::setProjMatrix(const sharedlib::Matrix4& matrix)
{
